        ...
        <div class="page-header pb-2">
            ...
            <div class="page-tools d-inline-flex">
                <a data-toggle="tooltip" data-placement="top" title="Refresh"
                    href="{{ route('admin.editjdih', $updatejdih->id) }}" type="button"
            ...
            <div class="col-lg-12 mb-4 mb-lg-0">
                <div class="card acard mt-lg-12 border-t-4 brc-blue-tp3">
                    <div class="card-body pb-1">
                        <form action="{{ route('admin.rubahjdih', $updatejdih->id) }}" method="POST"
                            enctype="multipart/form-data">
                            @csrf

                            <div class="form-group">
                                <label for="id-form-field-1" class="mb-0">
                                    Judul JDIH<span class="text-danger"> *)</span>
                                </label>
                                <input name="juduljdih" value="{{ $updatejdih->juduljdih }}" type="text"
                                    class="form-control" id="exampleInputEmail1" placeholder="Silahkan isikan judul JDIH">
                                @error('juduljdih')
                            ...
                            <div class="form-group">
                                <label for="id-form-field-1" class="mb-0">
                                    Keterangan JDIH<span class="text-danger"> *)</span>
                                </label>
                                <div class="card bcard border-1 brc-dark-l1">
                                    <div class="card-body p-0">
                                        <textarea name="keteranganjdih" id="summernote" name="editordata">
                                            {{ $updatejdih->deksripsijdih }}
                                        </textarea>
                                    </div>
                                </div>
                                @error('keteranganjdih')
                                    <em class="text-danger text-80">{{ $message }}</em></p>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                <label for="">
                                    Status JDIH <span class="text-danger">*)</span>
                                </label>

                                <select name="statusjdih" id="form-field-chosen-1" data-placeholder="Pilih Status JDIH.."
                                    class="chosen-select form-control col-lg-12 col-md-6">
                                    <option value="{{ $updatejdih->status }}">{{ $updatejdih->status }}</option>
                            ...
                            <div class="form-group">
                                <label for="id-form-field-1" class="mb-0">
                                    File JDIH <span class="text-danger"> *)</span>
                                </label>

                                <input id="fileanggaran" type="file" name="filejdih" value="{{ old('filejdih') }}"
                                    class="form-control" accept=".pdf" multiple />
                                @error('filejdih')
                                    <em class="text-danger text-80">{{ $message }}</em></p>
                                @enderror

                                <div class="card-body p-0 shadow">
                                    <iframe src="{{ asset('dokumen jdih/' . $updatejdih->filejdih) }}" frameborder="0"
                                        class="mt-2" width="100%" height="600px"></iframe>
                                </div>
                            </div>
