    ...
    <main id="info_publik" class="bg-F2F2F2 px-3 px-md-4 px-xl-5 py-3 py-md-4">
        <div class="wrapall">
            ...
            <div class="subrik d-flex flex-wrap gap-2 py-2 mb-3 mb-lg-4">
                @foreach ($groupanggaran as $item)
                    <a href="{{ route('perkategorianggaran', $item->slug_kategorianggaran) }}">
            ...
            <div class="bg-white shadow radius-respon w-100 py-3 py-lg-4">
                <ul class="px-4" style="list-style:none;">
                    @foreach ($transparansianggaran as $index => $item)
                        <li class="pb-2">
                            <i class="fas fa-check-circle text-E8BF70"></i>
                            <a class="hover-bb-dark link-dark text-decoration-none"
                                href="{{ asset('dokumen anggaran/' . $item->dokumenanggaran) }}" target="_blank">
                                {{ $item->judulanggaran }}
                            </a>
                            <a href="{{ asset('dokumen anggaran/' . $item->dokumenanggaran) }}" class="btn text-danger"
                                title="Klik Untuk Preview" target="_blank">
