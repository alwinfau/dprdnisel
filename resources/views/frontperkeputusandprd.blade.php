                    ...
                    <div class="ps-3 pe-4 py-3 py-lg-4">
                        <ul class="ul2">
                            ...
                            @foreach ($ketegorikeputusan as $item)
                                <a href="{{ route('keputusandprd', $item->slugkategori) }}">
                                    {{ $no++ }}. {{ $item->kategori }}
                                </a>
                            @endforeach
                        </ul>
                    ...
                    <div class="px-3 px-lg-4 py-3 py-lg-4">
                        <table class="ppid w-100 mb-3 mb-lg-4">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th class="judul">Nama File Perturan PPRD</th>
                            ...
                            <tbody>
                                @php
                                    $nomor = 1;
                                @endphp
                                @foreach ($perkeputusan as $index => $item)
                                    <tr>
                                        <td>{{ $index + $perkeputusan->firstItem() }}</td>
                                        <td class="judul">
                                            {{ $item->filekeputusan }}
                                        </td>
                                        <td class="text-center">
                                            <a class="btn btn-danger btn-md"
                                                href="{{ asset('peraturan dprd/' . $item->filekeputusan) }}" target="_blank"
                                                title="Lihat Dokumen">
