    ...
    <main id="ppid" class="bg-F2F2F2 px-3 px-md-4 px-xl-5 py-3 py-md-4">
        <div class="wrapall">
            <div class="d-flex align-items-center mb-2">
                ...
                <a href="{{ route('keputusandprd') }}" class="link-dark hover-bb-dark">
                    Keputusan DPRD
                </a>
            </div>
                    ...
                    <div class="ps-3 pe-4 py-3 py-lg-4">
                        <ul class="ul2">
                            @php
                                $no = 1;
                            @endphp
                            @foreach ($ketegorikeputusan as $item)
                                <a href="{{ route('perkeputusan', $item->slugkategori) }}">
                                    {{ $no++ }}. {{ $item->kategori }}
                                </a>
                            @endforeach
                        </ul>
                    ...
                    <div class="px-3 px-lg-4 py-3 py-lg-4">
                        <table class="ppid w-100 mb-3 mb-lg-4">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th class="judul">Nama File Keputusan PPRD</th>
                            ...
                            <tbody>
                                @php
                                    $nomor = 1;
                                @endphp
                                @foreach ($semuakeputusan as $index => $item)
                                    <tr>
                                        <td>{{ $index + $semuakeputusan->firstItem() }}</td>
                                        <td class="judul">
                                            {{ $item->filekeputusan }}
                                        </td>
                                        <td class="text-center">
                                            <a class="btn btn-danger btn-md"
                                                href="{{ asset('keputusan dprd/' . $item->filekeputusan) }}" target="_blank"
                                                title="Lihat Dokumen">
