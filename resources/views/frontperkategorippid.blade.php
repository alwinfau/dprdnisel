    ...
    <main id="ppid" class="bg-F2F2F2 px-3 px-md-4 px-xl-5 py-3 py-md-4">
        <div class="wrapall">
            <div class="d-flex align-items-center mb-2">
                ...
                <a href="{{ route('frontppid') }}" class="link-dark hover-bb-dark">
                    PPID
                </a>
            </div>
                    ...
                    <div class="ps-3 pe-4 py-3 py-lg-4">
                        <ul class="ul2">
                            @php
                                $no = 1;
                            @endphp
                            @foreach ($groupppid as $item)
                                <a href="{{ route('perkategorippid', $item->slugkategorippid) }}">
                                    {{ $no++ }}. {{ $item->kategorippid }}
                                </a>
                            @endforeach
                        </ul>
                    </div>
                    ...
                    <div class="px-3 px-lg-4 py-3 py-lg-4">
                        <table class="ppid w-100 mb-3 mb-lg-4">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th class="judul">Nama Dokumen</th>
                            ...
                            <tbody>
                                @php
                                    $nomor = 1;
                                @endphp
                                @foreach ($perkategorippid as $item)
                                    <tr>
                                        <td>{{ $nomor++ }}</td>
                                        <td class="judul">
                                            {{ $item->judulppid }}
                                        ...
                                        <td class="text-center">
                                            <a class="btn btn-danger btn-md"
                                                href="{{ asset('dokumen ppid/' . $item->fileppid) }}" target="_blank"
                                                title="Lihat Dokumen">
